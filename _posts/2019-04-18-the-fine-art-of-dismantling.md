...
synopsis: "What happens when there is the need to write data in a distributed system? What if these data need to go to different services? How do services participate in this process like they do for the Composition part? This is when ViewModel Decomposition comes into play. Let's have a look at what it is and especially when it's really needed."
tags:
- SOA
...
The last nine articles were focused on composing ViewModels to present data coming from different services. In all these articles, available in the [ViewModel Composition category](/categories/view-model-composition.html), data flow from the system to users. But this isn't the only direction in which data flow in a system. Users add and edit data all the time.
...
First thing, first. It's very easy to immediately jump to a solution for the above requirement:

> We need a *product* page that allows to edit *product* details and finally saves them talking to each of the services. Same goes for a page that allows to create a new *product*.
...
None of the above intentions/actions include changing an entire *product* at once. In order to sell a new *product* by adding it to the catalog we first need to buy it from a supplier. Warehouse has a back-office page that allows to add a new supplied *product* with details such as:
...
A page in Sales allows to create the Sales concept of a *product*, with the same ID of the Warehouse one, with an assigned price. A page in Marketing allows to define name and description to assign to the *product* when it'll be available on the online catalog. And finally a different page, still in Marketing probably, allows to select which *products* are now available for sale online.
...
There are however scenarios that require users to "edit", at the same time, data owned by different services. Let's say that we need to reserve an hotel room online. We search for the room availability, select what we prefer and land on a "confirm reservation" page. On this page we need to input:
...
* Reservation should keep track of the reservation request but wait at least for Payments to confirm that the payments details are valid and the credit card has been charged.
...
The infrastructure we've designed so far can easily handle such scenario. The `IHandleRequests` is designed to handle read requests, but also write requests:
...
Payments can define a request handler that intercepts HTTP post requests to URL like `/reservation/{reservation-id}`, the same can be done by Customer Care, and by Reservation.
...
Customer Care simply stores customer information along with the `reservation-id` to keep track that a reservation belongs to a customer. By using list composition techniques Customer Care can now handle scenarios like: we want to display all the reservation made by a customer.

Payments sends to its backend credit card details. Backend kicks off the payment process, that again is a state machine. Once done it'll notify the system, e.g. via an event on a queue, that the payment for a reservation identified by a specific id succeeded (or failed).

Reservation will react to the event and will transition its state machine from `reservation-pending` to `reservation-confirmed`, or `reservation-declined` in case of payment failure.

In order to do so, all what the handlers deployed by services need to do is to extract data from the incoming POST request:
...
Sending data to backend it's not so easy at it seems at a first look. The incoming request is HTTP and there are, in this sample, three different handlers that try to handle part of request. What if one of them fails? We need to find a way to deal with the fact that HTTP has no transactions support. The next article will be focused on what we can do when things go wrong.
